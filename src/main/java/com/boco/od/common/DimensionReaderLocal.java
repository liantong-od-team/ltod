 ...
 */
public class DimensionReaderLocal implements Serializable {
...
//    private int LRC_INDEX_DAY;
    ...
    public DimensionReaderLocal(Configuration conf) {
        this.conf = conf;
...
/*        long d1 = System.currentTimeMillis();
        ...
        //local 模式，加载datas
        if ("local".equalsIgnoreCase(join_type)) {
            GisTool.getInstance().load(GisTool.COUNTRY_FILE_PATH);
        }
        System.out.println(" load country cache time cost:"+(System.currentTimeMillis()-d1));*/
                    ...
                    while (null != (info = br.readLine())) {
                        ...
                        if (parts.length >= cell_columnSize) {
                            ...
                            if (provinc != null && (provinc.trim().equals("011") || provinc.trim().equals("013") || provinc.trim().equals("018"))) {
                                /*try {
                                    if ("local".equalsIgnoreCase(join_type)) {
                                        System.out.println("do local-> GisTool.getInstance().getCountry");
                                        country = GisTool.getInstance().getCountry(parts[CELL_INDEX_LONGITUDE], parts[CELL_INDEX_LATITUDE]);
                                        cell_localfile_succ++;
                                    } else if ("oracle".equalsIgnoreCase(join_type)) {
                                        System.out.println(String.format("get oracle= %s %s", parts[CELL_INDEX_LONGITUDE], parts[CELL_INDEX_LATITUDE]));
...
//                                System.out.print("map-key="+parts[CELL_INDEX_LAC] + "-" + parts[CELL_INDEX_CELL_ID]);
//                                System.out.println("\tmap-v="+parts[CELL_INDEX_LONGITUDE]+" | "+parts[CELL_INDEX_LATITUDE]+" | "+ parts[CELL_INDEX_PROVINCE]

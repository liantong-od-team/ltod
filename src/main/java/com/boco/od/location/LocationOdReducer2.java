 ...
 */
public class LocationOdReducer2 extends Reducer<UserTimePair, Text, NullWritable, Text> {
        ...
        for (Text val : values) {
            ...
            if ("".equals(prevDay)) {
                ...
                if (Util.isTheSameDay(prevStart_date, start_date)) {
                    ...
                    if (!Util.isEndOfDay(prevStart_date)) {
                        ...
                        /*
                         不移动节点，此时 prev仍指向小于235959 时间的那个节点。
                          */
                    ...
                    if (!Util.isStartOfDay(start_date)) {
                        //补current节点的000000值,补的是变化点，O使用prev节点，D区域使用current节点！
                        genRecord("跨天 cur", ctx, level, "cur", day_start_time);
                    }
                    //当前节点数据移动到prev
                    this.save2Prev(0);
            ...
            if (!values.iterator().hasNext()) {
                if (Util.isStartOfDay(start_date)) {
                    ...
                    genRecord("最后一条", ctx, level, "cur", day_end_time);

                } else if (Util.isEndOfDay(start_date)) {
                    if(!Util.isEndOfDay(prevStart_date)){
                        //是 235959点的数据
                        //需要补 current 节点000000值
                        genRecord("最后一条", ctx, level, "cur", day_start_time);
                    }
                } else {
                    //是一天 中间点的数据，需要向前/向后补齐一天的数据
                    //需要补 current 节点000000值
                    genRecord("最后一条", ctx, level, "cur", day_start_time);
                    //补 current 节点235959值
                    genRecord("最后一条", ctx, level, "cur", day_end_time);

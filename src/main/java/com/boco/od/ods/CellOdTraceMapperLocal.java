 ...
 */
public class CellOdTraceMapperLocal extends Mapper<LongWritable, Text, OdTracePair, OdTraceRecord>{
    ...
    protected void setup(Context context) {
        ...
        DimensionReaderLocal reader = new DimensionReaderLocal(context.getConfiguration());
...
//        lrcMap = new HashMap<String, DimensionLrc>();
        ...
        context.getCounter("RELATIONS","CELL_LOCALFILE_SUCC").increment(reader.getCell_localfile_succ());
        context.getCounter("RELATIONS","CELL_ORACALE_SUCC").increment(reader.getCell_oracale_succ());
        context.getCounter("RELATIONS","CELL_GET_ERROR").increment(reader.getCell_get_error());
...
//        FF：未定义业务话单"
        ...
        //号码为空
        if(msisdn==null||"".equals(msisdn.trim())||msisdn.trim().length()<7||msisdn.trim().length()>11||!msisdn.trim().startsWith("1")){
            context.getCounter(COUNTER.Null).increment(1);
            System.out.println("msisdn==null ,length=");
            return;
        }
        if(CELL_ID_VAL==null||"".equals(CELL_ID_VAL.trim())){
            context.getCounter(COUNTER.Null).increment(1);
            System.out.println("CELL_ID_VAL=null");
            return;
        }
        if(LAC_VAL==null||"".equals(LAC_VAL.trim())){
            context.getCounter(COUNTER.Null).increment(1);
            System.out.println("LAC_VAL=null");
            return;
        }

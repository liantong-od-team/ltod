 ...
 */
public class CellOdTraceDriver1 extends Configured implements Tool {
...
//        println(conf);
        job.setJarByClass(CellOdTraceDriver1.class);

        job.setJobName("CellOdTrace");
        job.setMapperClass(CellOdTraceMapper1.class);
...
//        String [] testargs = new String[]{in,out,cell,lrc};
        try {
            int ret = ToolRunner.run(new CellOdTraceDriver1(), args);
            System.exit(ret);
        } catch (Exception e) {
            e.printStackTrace();
        }

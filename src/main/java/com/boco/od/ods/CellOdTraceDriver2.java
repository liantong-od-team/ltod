 ...
 */
public class CellOdTraceDriver2 extends Configured implements Tool {
...
//        println(conf);
        job.setJarByClass(CellOdTraceDriver2.class);

        job.setJobName("CellOdTrace");
        job.setMapperClass(CellOdTraceMapper2.class);
...
//        String [] testargs = new String[]{in,out,cell,lrc};
        try {
            int ret = ToolRunner.run(new CellOdTraceDriver2(), args);
            System.exit(ret);
        } catch (Exception e) {
            e.printStackTrace();
        }

 ...
 */
public class CellOdTraceDriverLocal extends Configured implements Tool {
...
//       conf.set("hadoop.tmp.dir", "D:\\odtest\\tmp\\");
        ...
        if(args.length>6){
            conf.set("join_type",args[6]);
        }
        initRelationConf(cellpath,numberpath,conf);//加载缓存数据
        Job job = new Job(conf);

//        println(conf);
        job.setJarByClass(CellOdTraceDriverLocal.class);

        job.setJobName("CellOdTrace");
        job.setMapperClass(CellOdTraceMapperLocal.class);
...
//        }
    ...
    public static void disCache(String dimDir, Configuration conf)  {
        FileSystem  fs = null;
        try {
